...
import { X, Hash, MapPin, Activity, Maximize2, Minimize2, HelpCircle } from 'lucide-react';
...
const DetailPanel: React.FC = () => {
  ...
  return (
    <div 
      className={`fixed top-4 right-4 z-20 bg-white/90 backdrop-blur-2xl border-l border-t border-b border-slate-200 rounded-xl shadow-2xl h-[calc(100vh-2rem)] overflow-y-auto custom-scrollbar flex flex-col ring-1 ring-black/5 transition-all duration-300 ease-in-out ${isExpanded ? 'w-[800px]' : 'w-96'}`}
    >
       {/* Header */}
       <div className="relative p-6 pb-4 border-b border-slate-100">
        ...
        <div className="absolute top-4 right-4 flex gap-2">
          <button 
            ...
            className="p-1 text-slate-400 hover:text-slate-900 transition-colors bg-slate-100 rounded-full hover:bg-slate-200"
           ...
           <div className="flex justify-between items-center mb-2">
             <div className="group relative flex items-center gap-2 cursor-help">
               ...
               <div className="absolute bottom-6 left-0 w-48 bg-slate-800 text-white text-[10px] p-2 rounded shadow-lg opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-50 leading-relaxed font-light">
                 A computed metric (1-10) representing the entity's structural centrality and semantic weight within the current network topology.
               </div>
             </div>
